...
(e.g., see \citet{hm1981})\ee{$\leftarrow$ this sentence needs edit: "it was seen to energy\ldots"}.
...
\ee{The continued accretion, which sustains the neutrino luminosity while the explosion develops is also important.}
...
multiple GPUs.
\ee{Need to discuss the major physics components that are coupled together to model CCSNe.}
Several production codes are currently used to simulate CCSNe, including
\flashx\ \citep{for2000},\ee{also cite Dubey et al.~(2022) for \flashx}
\cocov\ \citep{mjd2010},
...
\ee{It can be argued that DG methods are favored for modeling neutrino transport, and to avoid complications when coupling hydro and neutrino transport, this motivates the development of DG methods for hydro.  DG methods are also able to capture shocks, which makes them preferred over continuous finite-element methods (often used in CFD applications).}

CCSN dynamics occur on different length scales that span several orders of magnitude.  \ee{Provide an example, e.g., gravitational collapse.}
...
ways.
One way is known as ``patch-based'' AMR \ee{``block-structured''?}, where chunks of the mesh, spanning
...
\ee{Give a brief explanation of the effective potential approach.}
...
\ee{Offer some criticism of the effective-potential approach(?): It is ad-hoc, and not based on a self-consistent theory of gravity}
...
to run on GPUs.  \ee{And the coupling of the hydro solver to \poseidon, to enable, for the first time (?), general relativistic simulations of gravitational collapse with DG/CG methods, AMR, and tabulated nuclear EoS.}

\ee{As a consequence of this, we are now in the position to realize general relativistic neutrino hydrodynamics.}
...
using spectral methods \ee{spectral methods?} for the neutrino transport.
...
This document \ee{thesis?} is structured as follows: Chapter~\ref{ch.sasi} is an adaptation
of an accepted article \ee{accepted where?!} on the effects of GR on the SASI,
Chapter~\ref{ch.mp} is an adaptation of an article in preparation \ee{for which journal?} on our
