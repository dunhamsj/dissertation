...
Using GR hydrodynamics \repl{and}{with GR} gravity, and non-relativistic (NR)
hydrodynamics \repl{and}{with Newtonian} gravity,
...
We investigate \repl{two regimes}{four compactnesses} expected in a
post-bounce core-collapse supernova
(CCSN)\repl{: one meant to resemble a relatively low-compactness configuration
and one meant to resemble a relatively high-compactness configuration.}{.}
...
for the \repl{high}{highest}-compactness suite.
...
for the \repl{high}{highest}-compactness suite. We discuss implications of our
...
it would be \repl{impossible}{very difficult}
...
This perturbation generates more\repl{}{, stronger,} vortical waves,
which advect to
...
\repl{}{Indeed, \citet{dem2020} presented GR steady-state solutions and
presented preliminary results
\repl{}{from GR SASI simulations}, but did not compare results from NR and GR
simulations.}
...
\repl{under two sets of conditions:
one set is mildly relativistic and is designed to mirror
low-compactness conditions after bounce in a CCSN,
and the other set is strongly
relativistic and is designed to mirror high-compactness conditions.}
...
nonlinear regime,
we perform our assessment via \repl{two suites of}{seven} axisymmetric numerical
simulations\repl{, differentiated by their compactness,}{}
...
\repl{}{We will often use ``NR" to refer to the case of Newtonian gravity
and NRHD.}
...
with $\psi$ given by \myeqref{eq.psi}.
...
\myeqref{eq.polytrope} is consistent with \myeqref{eq.idealEOS}
through the first law of thermodynamics for an isentropic fluid.

\subsection{Non-Relativistic Hydrodynamics}

Under the 3+1 formalism of GR \repl{}{and the CFC},
...
\repl{NR}{Newtonian gravity},
the metric is that of flat space and the effect of gravity
is encoded in the Newtonian gravitational potential,$\Phi$\repl{.}{,
\begin{equation}
...
\repl{.}{and $\psi=1-\Phi/2$.
}

In the case of \repl{NR}{Newtonian gravity and NRHD}, we solve\begin{equation}
  \p_{t}\,\bs{U}
...
solutions to \myeqref{eq.cLaws} (GR) and \myeqref{eq.cLawsNR} (NR).
...
in \myeqref{eq.cLaws} yields the relativistic Bernoulli equation,
\begin{equation}
  \alpha\,h\,W=\mc{B},
  \label{IC.eq2}
\end{equation}
...
As in the GR case, we close this system with \myeqref{eq.polytrope}.

In the non-relativistic limit,
$\alpha\approx1+\Phi$ and $W\approx1+\frac{1}{2}\,v^{i}\,v_{i}$;
substituting these into \myeqref{IC.eq2} yields, to leading order,
$1+\mc{B}_{\nr}$,
in agreement with \myeqref{eq.BernoulliNR}.

\subsection{Comparison of NR and GR Steady-State Solutions}

For the \repl{low-compactness case}{two lower-compactness cases
(see \myeqref{eq.compactness})}
\citep[e.g., see][]{bmh2013,mjm2015,brv2020},
\figref{fig.compSS_loXi} shows steady-state accretion shock solutions
as functions of coordinate distance $r$\repl{,
with $\mpns=1.4\,\msun$ (where $\msun$ is the mass of our Sun),
$\mdot=0.3\,\msun\,\s^{-1}$,
and $\rsh/\km\in\left\{120,150,175\right\}$}{}.
\repl{}{(Note that in \figref{fig.compSS_loXi} and \figref{fig.compSS_hiXi} below,
...
\citep[\repl{see, e.g.,}{e.g., see} Eqs. (1-3) in][]{bmd2003}.
...
For the \repl{high-compactness case}{two higher-compactness cases}
\citep[e.g., see][]{lmt2001,wtj2020},
\figref{fig.compSS_hiXi} shows steady-state accretion shock solutions
as functions of coordinate distance $r$\repl{,
with $\mpns=2.8\,\msun$, $\mdot=0.3\,\msun\,\s^{-1}$,
and $\rsh/\km\in\left\{60,70\right\}$}{}.
...
for three models having the same
accretion rate of 0.3$\,\msun/\s$,
\repl{the same PNS mass of $2.8\,\msun$, and shock radii
60 km (blue) and 70 km (orange)}
...
For \repl{this PNS mass}{the $2.8\,\msun$ PNS models},
...
to circumnavigate the \repl{PNS}{post-shock cavity}
...
\repl{}{The expression for $\tac$ is inspired by the Lamb frequency,
which relates the frequency of an acoustic wave to the radius at which it turns
around \citep{hkt2004}.
We delay the specification of
$\rac$ until \secref{sec.results}.}
...
\repl{high-compactness suite}{higher-compactness models} and for smaller
radii.
\begin{figure}[htb!]
  \centering
  \includegraphics[width=0.8\textwidth]%
...
\repl{Low}{Lower}-compactness models are shown in the top panel
and \repl{high}{higher}-compactness models
...
The rough estimates provided by \myeqref{eq.MullerEst} and \myeqref{eq.Tac} are
...
see \citet{ebd2019} and \citet{pbe2021}\repl{.
In}{; for} the relativistic case, see \citet{dem2020} and
...
\end{equation}
\repl{One suite, which we refer to as ``low-compactness", has $\xi=0.7$;
the other suite, which we refer to as ``high-compactness", has $\xi=2.8$.
We vary $\mpns$ and $\rpns$ in such a way as to ensure these two values
of compactness.}{}
\input{ModelParametersTable.tex}
...
\myeqref{eq.idealEOS}, \myeqref{eq.polytrope}, the assumption of $\Gamma=4/3$,
and the assumption of a small velocity at the PNS surface.

For the
\repl{low-compactness conditions}{$\xi\in\left\{0.4,0.7\right\}$ models}
...
For the
\repl{high-compactness conditions}{$\xi\in\left\{1.8,2.8\right\}$ models}
we enforce a radial resolution of
...
as in the \repl{low-compactness}{$\xi\in\left\{0.4,0.7\right\}$} models,
...
for
\repl{the low-compactness}{}
model \texttt{GR1D\_M1.4\_Rpns040\_Rsh1.20e2} (top panel)
and \repl{the high-compactness}{} model
...
versus $t/\tauad$ for \repl{low-compactness}{} model
\texttt{GR1D\_M1.4\_Rpns040\_Rsh1.20e2}
(top panel)
and \repl{high-compactness}{} model \texttt{GR1D\_M2.8\_Rpns020\_Rsh6.00e1}
(bottom panel); other models exhibit similar behavior.
\repl{}{For example, in the top panel,}
it can be seen that the \repl{low-compactness}{}
...
of the mass density versus $t/\tauad$ for \repl{low-compactness}{} model
\texttt{GR1D\_M1.4\_Rpns040\_Rsh1.20e2} (top panel)
and \repl{high-compactness}{} model
\texttt{GR1D\_M2.8\_Rpns020\_Rsh6.00e1} (bottom panel).}
  \label{fig.relax}
\end{figure}
...
\begin{equation}
  ...
  \cos\theta,
\end{equation}
with $\eta$ defined in \myeqref{eq.eta},
...
where, for simplicity, we use \myeqref{eq.MullerEst} to define the period of one
SASI oscillation.
...
Here we discuss trends that appear across \repl{both the low-compactness models
and the high-compactness}{our} models.
...
(estimated using \myeqref{eq.MullerEst}),
and the period assuming a purely acoustic mechanism\repl{}{, $\tac$}
(estimated using \myeqref{eq.Tac}).
\repl{}{When using \myeqref{eq.Tac}, we choose the characteristic radius $\rac$
to be $0.85\,\rsh$, which is the midpoint of the shell in which we compute
the power.}
\repl{Due to data uncertainties, and the similar values provided by the two estimates,
we are unable to discern whether the SASI is governed by an
advective-acoustic mechanism or by a purely acoustic mechanism using these
simple estimates.}{}
...
amplitudes of the Fourier transform of \myeqref{eq.F12}, $\wt{F}^{r}_{\theta}$,
versus $\wt{T}$ for \repl{all}{four} models,
...
\begin{figure}[htb!]
  ...
  \end{minipage}
  \caption{
Amplitudes of Fourier transforms of \myeqref{eq.F12},
...
\repl{From left to right, columns correspond to models with increasing
shock radii
and rows corresponds to low-compactness (top) and high-compactness (bottom)
models.}{The compactness of each model is shown in the bottom of the
respective panels.}
...
\repl{estimates for the period provided by
Eqs. (27) and (30) along with the period}{SASI period},
\repl{ and its uncertainty}{, as}
provided by the Fourier analysis,
versus initial shock radius for all \repl{the}{of our} models.
\begin{figure}[htb!]
  \centering
  \includegraphics[width=0.8\textwidth]%
  {fig.OscillationPeriodComparison.pdf}
  \caption{
SASI oscillation period \repl{and uncertainty}{} in $\ms$ as a function of
\repl{}{(dimensionless)} PNS \repl{mass}{compactness} and
shock radius in $\km$, as determined by the FFT (see text for details).
NR results are shown in \repl{blue}{open squares}
and GR results are shown in \repl{orange}{solid squares,
with different colors corresponding to different compactnesses}.
\repl{Crosses correspond to estimates of the period assuming the SASI is governed
by an advective-acoustic mechanism}{} (\myeqref{eq.MullerEst})
\repl{using the GR data and the GR part of}{} \myeqref{eq.MullerEst}
\repl{(orange)
and using the NR data and the NR part of}{} \myeqref{eq.MullerEst}\repl{ (blue).
Filled circles correspond to estimates of the period assuming the SASI
is governed by a purely acoustic mechanism}{} (\myeqref{eq.Tac}).}
  \label{fig.PC}
\end{figure}
The NR and GR models\repl{, along with their associated estimates,}{}
follow the same general trends.
\repl{For the low-compactness models,
we find that the ratio between
$T_{\gr}$ and $T_{\nr}$ is as large as \PeriodRatioGRoverNRxiB,
the relative difference between $T_{\gr}$ and the advective-acoustic
estimate, Eq.~(27), is smaller than \RelDiffAAxiB,
and the relative difference between $T_{\gr}$ and the purely acoustic
estimate, Eq.~(30), is smaller than \RelDiffACxiB.
For the high-compactness models,
we find that the ratio between
$T_{\gr}$ and $T_{\nr}$ is as large as \PeriodRatioGRoverNRxiD,
the relative difference between $T_{\gr}$ and the advective-acoustic
estimate, Eq.~(27), is smaller than \RelDiffAAxiD,
and the relative difference between $T_{\gr}$ and the purely acoustic
estimate, Eq.~(30), is smaller than \RelDiffACxiD.}{
...
\repl{}{For the $\xi=0.4,0.7,1.8$, and $2.8$ models,
the relative differences between $T_{\gr}$ and the advective-acoustic
estimate, \myeqref{eq.MullerEst}, are
\RelDiffACxiA,
$<\RelDiffACxiB$,
\RelDiffACxiC,
...
as a function of time in milliseconds for \repl{all our models}
...
The layout of the panels is the same as in \figref{fig.fft}\repl{:
From left to right, columns correspond to models with increasing
shock radii
and rows corresponds to low-compactness (top) and high-compactness (bottom)
models.}.
NR results are shown in blue and GR results are shown in orange.
...
less than 10\%,
\repl{and we consider this sufficient}{which we consider sufficient} for
...
In \figref{fig.H1}, it can be seen that\repl{, even for the
low-compactness models,
GR has a non-negligible effect on the growth rate}{
the growth rate for the $\xi=0.4$ model is practically unaffected by GR,
...
\repl{going from the low-compactness models
to the high-compactness models}{looking at the $\xi=2.8$ models}.
In \repl{the high-compactness}{these} models,
...
\repl{.}{, which is the result of a factor of two difference
...
as determined by the fit to \myeqref{eq.H1Fit} (see text for details).
NR results are shown in \repl{blue}{open squares}
and GR results are shown in \repl{orange}{solid squares}.}
  \label{fig.GRC}
\end{figure}
...
In \figref{fig.H1t}, we \repl{again}{} plot $H_{1}$\repl{,
but this time as a function of $t/T$.}
...
(When interpreting \repl{this figure}{the right panels},
...
\repl{$t/T$. The}{$t/\ms$ (left) and versus $t/T$ (right).
In each four-panel figure, the}
top-left panel shows the
\repl{low-compactness}{$\xi\in\left\{0.4,0.7\right\}$}, NR models;
the top-right panel shows the
\repl{low-compactness}{$\xi\in\left\{0.4,0.7\right\}$}, GR models;
the bottom-left panel shows the
\repl{high-compactness}{$\xi\in\left\{1.8,2.8\right\}$}, NR models;
and the bottom-right panel shows the
\repl{high-compactness}{$\xi\in\left\{1.8,2.8\right\}$}, GR models.}
  \label{fig.H1t}
\end{figure}
\repl{This figure demonstrates}
{These figures demonstrate} that the \repl{growth rate
per oscillation period, $\omega\,T$,}{SASI cycle efficiency}
is approximately constant for a given physical model (NR or GR) and compactness.
\repl{}{This can be seen by, for example, first looking at the top-left panel of the left
...
Further, all the \repl{low-compactness models (both NR and GR)
and the NR high-compactness}
{NR models and the $\xi\in\left\{0.4,0.7\right\}$ GR} models reach
about the same total power of $\sim$10$^{25}$\repl{}{-$10^{26}$}
after ten oscillations,
while the \repl{high-compactness}{$\xi\in\left\{1.8,2.8\right\}$}
...
\repl{$\omega\,T$}{the SASI efficiency} roughly constant for
each of these groupings of models.

\repl{}{The SASI efficiency is further illustrated in
...
\repl{For the low-compactness ($\xi=0.7$) models,
we found that the period of the SASI in the GR case is
larger than that of the SASI in the NR case, with a ratio
between the two as large as $\sim$\PeriodRatioGRoverNRxiB.
For the high-compactness ($\xi=2.8$) models,
we found that the period of the SASI in the GR case is
larger than that of the SASI in the NR case, with a ratio
between the two as large as $\sim$\PeriodRatioGRoverNRxiD,
...
\repl{Our numerically-determined oscillation periods
are consistent with
simple estimates assuming
both advective-acoustic and purely
acoustic mechanisms,
however, due to uncertainties in numerically-determined
oscillation periods,
we cannot discern between the two with our data.}{}
For all models, we found that the
growth rate is slower when GR is used,
and significantly slower for the
\repl{high-compactness}{$\xi=1.8$ and $\xi=2.8$} models,
...
\repl{core-collapse supernova (CCSN)}{CCSN}
...
become increasingly relativistic\repl{}{; i.e., increased PNS compactness
...
the SASI growth rate continued to increase (\figref{fig.GRC}) and its period
continued to decrease (\figref{fig.PC}).
...
\repl{The data underlying this paper will be shared on reasonable request to the
...
%\dataset[here]{https://doi.org/10.5281/zenodo.10420763}.
}

\repl{}{All simulations presented here have been performed
